	...
	d3.csv("../data/postSeason.csv", function(error,csvdata){
	  ...
	  svg.selectAll('.TimeLine')
	    ...
	    function drawPath( color, X0, Y0, X1, Y1, startWidth, endWidth,team){
	      ...
	      var dataset = [X0];
	      var index = 0;
	      var pow1 = d3.scalePow()
	                  .exponent(1.9)
	                  .domain([0,(X1-X0)/2])
	                  .range([Y0,(Y0+Y1)/2]);
	      var pow2 = d3.scalePow()
	                  .exponent(1/1.9)
	                  .domain([0,(X1-X0)/2])
	                  .range([(Y0+Y1)/2,Y1]);

	      var pow3 = d3.scalePow()
	                  .exponent(2)
	                  .domain([X0,X1])
	                  .range([startWidth,
	                    endWidth]);
	      //跨两格
	      var pow4 = d3.scalePow()
	                  .exponent(1/1.5)
	                  .domain([0,(X1-X0)/2])
	                  .range([Y0,0.85*(Y0+Y1)]);

	      var pow5 = d3.scalePow()
	                  .exponent(1.5)
	                  .domain([0,(X1-X0)/2])
	                  .range([0.85*(Y0+Y1),Y1]);
	    ...
	    line.select(function(d,j){ 
	        ...
	        drawPath('#fff', 101,115,203.30769230769232,157.69058295964126, 43.930635838150295, 37.30941704035874, 'SD');
	    ...
	    // svg.append('line')
	    //    .attr('x1', timeLine[10] + padding/2)
	    //    .attr('y1', padding)
	    //    .attr('x2',timeLine[10] + padding/2)
	    //    .attr('y2', 300)
	    //    .attr('fill', 'blue')
	    //    .attr('class', 'timeLine');;
	});
